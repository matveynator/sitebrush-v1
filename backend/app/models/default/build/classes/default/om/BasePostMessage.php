...
/**
 * Base class that represents a row from the 'post_message' table.
 *
 * 
 *
 * @package    propel.generator.default.om
 */
abstract class BasePostMessage extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'PostMessagePeer';
	 ...
	 * @return     PostMessage The current object (for fluent API support)
	 */
	public function setPostId($v)
	{
		...
		if ($this->post_id !== $v) {
			$this->post_id = $v;
			$this->modifiedColumns[] = PostMessagePeer::POST_ID;
		}
	...
	public function setMessageId($v)
	{
		...
		if ($this->message_id !== $v) {
			$this->message_id = $v;
			$this->modifiedColumns[] = PostMessagePeer::MESSAGE_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = PostMessagePeer::NUM_COLUMNS - PostMessagePeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating PostMessage object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = PostMessagePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				PostMessagePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedRows += PostMessagePeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = PostMessagePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = PostMessagePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['PostMessage'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['PostMessage'][serialize($this->getPrimaryKey())] = true;
		$keys = PostMessagePeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PostMessagePeer::POST_ID)) $criteria->add(PostMessagePeer::POST_ID, $this->post_id);
		if ($this->isColumnModified(PostMessagePeer::MESSAGE_ID)) $criteria->add(PostMessagePeer::MESSAGE_ID, $this->message_id);
	...
	{
		$criteria = new Criteria(PostMessagePeer::DATABASE_NAME);
		$criteria->add(PostMessagePeer::POST_ID, $this->post_id);
		$criteria->add(PostMessagePeer::MESSAGE_ID, $this->message_id);
	...
	{
		return (null === $this->getPostId()) && (null === $this->getMessageId());
	}
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of PostMessage (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     PostMessage Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     PostMessagePeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new PostMessagePeer();
		}
		return self::$peer;
	}
	...
	public function getPost(PropelPDO $con = null)
	{
		if ($this->aPost === null && ($this->post_id !== null)) {
			...
			/* The following can be used additionally to
				...
				$this->aPost->addPostMessages($this);
			 */
		}
		return $this->aPost;
	}
	...
	public function getMessage(PropelPDO $con = null)
	{
		if ($this->aMessage === null && ($this->message_id !== null)) {
			...
			/* The following can be used additionally to
				...
				undesirable since it could result in an only partially populated collection
				in the referenced object.
				$this->aMessage->addPostMessages($this);
			 */
		}
	...
	{
		return (string) $this->exportTo(PostMessagePeer::DEFAULT_STRING_FORMAT);
	}
