...
/**
 * Base class that represents a row from the 'post_media' table.
 *
 * 
 *
 * @package    propel.generator.default.om
 */
abstract class BasePostMedia extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'PostMediaPeer';
	 ...
	 * @return     PostMedia The current object (for fluent API support)
	 */
	public function setPostId($v)
	{
		...
		if ($this->post_id !== $v) {
			$this->post_id = $v;
			$this->modifiedColumns[] = PostMediaPeer::POST_ID;
		}
	...
	public function setMediaId($v)
	{
		...
		if ($this->media_id !== $v) {
			$this->media_id = $v;
			$this->modifiedColumns[] = PostMediaPeer::MEDIA_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = PostMediaPeer::NUM_COLUMNS - PostMediaPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating PostMedia object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($con === null) {
			$con = Propel::getConnection(PostMediaPeer::DATABASE_NAME, Propel::CONNECTION_READ);
		}

		// We don't need to alter the object instance pool; we're just modifying this instance
		// already in the pool.

		$stmt = PostMediaPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		if ($con === null) {
			$con = Propel::getConnection(PostMediaPeer::DATABASE_NAME, Propel::CONNECTION_WRITE);
		}
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				PostMediaPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedRows += PostMediaPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = PostMediaPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = PostMediaPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['PostMedia'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['PostMedia'][serialize($this->getPrimaryKey())] = true;
		$keys = PostMediaPeer::getFieldNames($keyType);
		$result = array(
	...
	{
		$criteria = new Criteria(PostMediaPeer::DATABASE_NAME);

		if ($this->isColumnModified(PostMediaPeer::POST_ID)) $criteria->add(PostMediaPeer::POST_ID, $this->post_id);
		if ($this->isColumnModified(PostMediaPeer::MEDIA_ID)) $criteria->add(PostMediaPeer::MEDIA_ID, $this->media_id);
	...
	{
		$criteria = new Criteria(PostMediaPeer::DATABASE_NAME);
		$criteria->add(PostMediaPeer::POST_ID, $this->post_id);
		$criteria->add(PostMediaPeer::MEDIA_ID, $this->media_id);
	...
	{
		return (null === $this->getPostId()) && (null === $this->getMediaId());
	}
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of PostMedia (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     PostMedia Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     PostMediaPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new PostMediaPeer();
		}
		return self::$peer;
	}
	...
	public function getMedia(PropelPDO $con = null)
	{
		if ($this->aMedia === null && ($this->media_id !== null)) {
			...
			/* The following can be used additionally to
				...
				undesirable since it could result in an only partially populated collection
				in the referenced object.
				$this->aMedia->addPostMedias($this);
			 */
		}
		return $this->aMedia;
	}
	...
	{
		return (string) $this->exportTo(PostMediaPeer::DEFAULT_STRING_FORMAT);
	}
