 ...
 */
abstract class BaseInvite extends BaseObject  implements Persistent
{
	 ...
	 * @return     Invite The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = InvitePeer::ID;
		}
	...
	public function setHash($v)
	{
		...
		if ($this->hash !== $v) {
			$this->hash = $v;
			$this->modifiedColumns[] = InvitePeer::HASH;
		}
	...
	public function setStatus($v)
	{
		...
		if ($this->status !== $v) {
			$this->status = $v;
			$this->modifiedColumns[] = InvitePeer::STATUS;
		}
	...
	public function setDomain($v)
	{
		...
		if ($this->domain !== $v) {
			$this->domain = $v;
			$this->modifiedColumns[] = InvitePeer::DOMAIN;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = InvitePeer::NUM_COLUMNS - InvitePeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Invite object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = InvitePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				InvitePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(InvitePeer::ID) ) {
					...
					$affectedRows = InvitePeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = InvitePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = InvitePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['Invite'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Invite'][$this->getPrimaryKey()] = true;
		$keys = InvitePeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getHash(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(InvitePeer::ID)) $criteria->add(InvitePeer::ID, $this->id);
		if ($this->isColumnModified(InvitePeer::HASH)) $criteria->add(InvitePeer::HASH, $this->hash);
		if ($this->isColumnModified(InvitePeer::STATUS)) $criteria->add(InvitePeer::STATUS, $this->status);
		if ($this->isColumnModified(InvitePeer::DOMAIN)) $criteria->add(InvitePeer::DOMAIN, $this->domain);
	...
	{
		$criteria = new Criteria(InvitePeer::DATABASE_NAME);
		$criteria->add(InvitePeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Invite (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Invite Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * If this Invite is new, it will return
	 ...
	 * Otherwise if this Invite is new, it will return
	 * an empty collection; or if this Invite has previously
	 * been saved, it will retrieve related UserInvites from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Invite.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	 ...
	 * @return     PropelCollection|array UserInvite[] List of UserInvite objects
	 */
	public function getUserInvitesJoinUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = UserInviteQuery::create(null, $criteria);
	 ...
	 *
	 * @param      User $user The UserInvite object to relate
	 * @return     void
	 */
	...
	{
		return (string) $this->exportTo(InvitePeer::DEFAULT_STRING_FORMAT);
	}
