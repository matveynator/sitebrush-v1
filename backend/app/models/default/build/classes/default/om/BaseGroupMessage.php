...
/**
 * Base class that represents a row from the 'group_message' table.
 *
 * 
 *
 * @package    propel.generator.default.om
 */
abstract class BaseGroupMessage extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'GroupMessagePeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        GroupMessagePeer
	 */
	 ...
	 * @return     GroupMessage The current object (for fluent API support)
	 */
	public function setGroupId($v)
	{
		...
		if ($this->group_id !== $v) {
			$this->group_id = $v;
			$this->modifiedColumns[] = GroupMessagePeer::GROUP_ID;
		}
	...
	public function setMessageId($v)
	{
		...
		if ($this->message_id !== $v) {
			$this->message_id = $v;
			$this->modifiedColumns[] = GroupMessagePeer::MESSAGE_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = GroupMessagePeer::NUM_COLUMNS - GroupMessagePeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating GroupMessage object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = GroupMessagePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				GroupMessagePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedRows += GroupMessagePeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = GroupMessagePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = GroupMessagePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['GroupMessage'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['GroupMessage'][serialize($this->getPrimaryKey())] = true;
		$keys = GroupMessagePeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(GroupMessagePeer::GROUP_ID)) $criteria->add(GroupMessagePeer::GROUP_ID, $this->group_id);
		if ($this->isColumnModified(GroupMessagePeer::MESSAGE_ID)) $criteria->add(GroupMessagePeer::MESSAGE_ID, $this->message_id);
	...
	{
		$criteria = new Criteria(GroupMessagePeer::DATABASE_NAME);
		$criteria->add(GroupMessagePeer::GROUP_ID, $this->group_id);
		$criteria->add(GroupMessagePeer::MESSAGE_ID, $this->message_id);
	...
	{
		return (null === $this->getGroupId()) && (null === $this->getMessageId());
	}
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of GroupMessage (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     GroupMessage Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     GroupMessagePeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new GroupMessagePeer();
		}
		return self::$peer;
	}
	...
	public function getGroup(PropelPDO $con = null)
	{
		if ($this->aGroup === null && ($this->group_id !== null)) {
			...
			/* The following can be used additionally to
				...
				$this->aGroup->addGroupMessages($this);
			 */
		}
		return $this->aGroup;
	}
	...
	public function getMessage(PropelPDO $con = null)
	{
		if ($this->aMessage === null && ($this->message_id !== null)) {
			...
			/* The following can be used additionally to
				...
				undesirable since it could result in an only partially populated collection
				in the referenced object.
				$this->aMessage->addGroupMessages($this);
			 */
		}
	...
	{
		return (string) $this->exportTo(GroupMessagePeer::DEFAULT_STRING_FORMAT);
	}
