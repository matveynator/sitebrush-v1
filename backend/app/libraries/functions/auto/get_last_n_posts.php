<?php
function GetLastNPosts($FinishTime, $Limit = 10, $PostType = "Blog", $OrderBy = "Date")
{
  ...
  if ((isset($_SESSION['UserId'])) and (isset($_SESSION['LoggedIn']) and ($_SESSION['LoggedIn'] == TRUE))) {
    ...
    if ($OrderBy == "LastComment") {
      $LastPosts = PostQuery::create()->filterByGroup($EveryoneGroup)->where('Post.LastComment <= ?', $FinishTime)->filterByShowOnFirst(1)->filterByType($PostType)->filterByStatus('Active')->filterByDomain($configuration['domain'])->orderByLastComment('Desc')->limit($Limit)->find();
    } else {
      $LastPosts = PostQuery::create()->filterByGroup($EveryoneGroup)->where('Post.Date <= ?', $FinishTime)->filterByShowOnFirst(1)->filterByType($PostType)->filterByStatus('Active')->filterByDomain($configuration['domain'])->orderByDate('Desc')->limit($Limit)->find();
    }
  }
  return $LastPosts;
}
