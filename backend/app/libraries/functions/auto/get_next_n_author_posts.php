<?php
function GetNextNAuthorPosts($Author, $StartTime, $Limit = 10, $PostType = "Blog", $OrderBy = "Date")
{
  ...
  if ((isset($_SESSION['UserId'])) and (isset($_SESSION['LoggedIn']) and ($_SESSION['LoggedIn'] == TRUE))) {
    ...
    if ($OrderBy == "LastComment") {
      $LastPosts = PostQuery::create()->filterByUser($Author)->filterByGroup($EveryoneGroup)->where('Post.LastComment > ?', $StartTime)->filterByType($PostType)->filterByStatus('Active')->filterByDomain($configuration['domain'])->orderByLastComment('Asc')->limit($Limit)->find();
    } else {
      $LastPosts = PostQuery::create()->filterByUser($Author)->filterByGroup($EveryoneGroup)->where('Post.Date > ?', $StartTime)->filterByType($PostType)->filterByStatus('Active')->filterByDomain($configuration['domain'])->orderByDate('Asc')->limit($Limit)->find();
    }
  }
  return $LastPosts;
}
